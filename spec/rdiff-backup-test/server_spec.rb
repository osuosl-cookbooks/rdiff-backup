    ...
    context "on #{pltfrm[:platform]} #{pltfrm[:version]}" do
      ...
      cached(:chef_run) { runner.converge(described_recipe) }
        ...
        before do
          ...
          Chef::EncryptedDataBagItem.stub(:load).with('rdiff-backup-secrets','secrets').and_return({
              key: 'secret-key'
          })
        end
        it do
        ...
        it do
          ...
          chef_run.node['nrpe']['plugin_dir'] + '/check_rdiff').with(
            mode: 0755,
        ...
        # it do
        #   expect(chef_run).to create_file('check_rdiff_job_test1').with(
        ...
        # end
          ...
          it do
            expect(chef_run).to create_directory('/var/rdiff-backup/locks').with(
              ...
              group: 'rdiff-backup-server' || 'rdiff-backup-server', # Group || Owner
              mode: 0755,
              recursive: true
            )
          end
          ...
          it do
            expect(chef_run).to create_file('/home/rdiff-backup-server/exclude/192.168.60.11/_help_me_obiwan').with(
              owner: 'rdiff-backup-server',
              group: 'rdiff-backup-server' || 'rdiff-backup-server',
              mode: 0644,
              content: ['**/darth-vader', '/help/me/obiwan/emperor-palpatine'].join("\n")
            )
          end
          it do
            expect(chef_run).to create_template('/home/rdiff-backup-server/scripts/192.168.60.11/_help_me_obiwan').with(
              source: 'job.sh.erb',
              mode: 0775,
              owner: 'rdiff-backup-server',
              group: 'rdiff-backup-server',
          ...
          it do
            expect(chef_run).to create_cron('test1').with(
              ...
              month:'*',
              user: 'rdiff-backup-server',
              command: '/usr/bin/flock /var/rdiff-backup/locks/test1 /home/rdiff-backup-server/scripts/192.168.60.11/_help_me_obiwan'
            )
          end
        end
      end
