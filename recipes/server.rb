...
for path in#{pathlist}; do \\
  rdiff-backup #{args} --force --create-full-path --exclude-device-files --exclude-fifos --exclude-sockets --remote-schema \\
    \"ssh -Cp #{port} -o StrictHostKeyChecking=no \\%s sudo rdiff-backup --server --restrict-read-only /\" \\
    \"#{user}\@#{fqdn}\:\:${path}\" \\
    \"#{destpath}\";
  rdiff-backup --force --remove-older-than #{period} \"#{destpath}\";
done"
        end
      else
